...
namespace pieventsnovo
{
    public class ParseArgs
    {
        ...
        public bool CheckHelpVersionOrEmpty()
        {
            ...
            if (args[0] == "-v" || args[0] == "-ver" || args[0] == "-version")
            {
                ...
                foreach (var a in assemblies)
                    sb.AppendLine($"{a.GetName().Name}, Assembly Ver. {a.GetName().Version}, " +
                        $"Product Ver. {FileVersionInfo.GetVersionInfo(new Uri(a.CodeBase).LocalPath).ProductVersion}");
        ...
        public static void PrintHelp(string msg)
        {
            ...
            sb.AppendLine("COMMAND \t USAGE <> = required [] = optional # description");
            sb.AppendLine("-snap <tagmasks> #current value");
            sb.AppendLine("-pointchanges <tagmasks> #point changes");
            sb.AppendLine("-sign,<[sa] or [t]> <tagmasks> s=snapshot, a=archive, sa=both, t=timeseries #signups");
            ...
            sb.AppendLine("-interp <tagmasks> <starttime,endtime>[,TimeSpan(hh:mm:ss) (def=10m)) or c=Count] #interpolated values");
            sb.AppendLine("-plot <tagmasks> <starttime,endtime>[,Intervals(def=640)] #plot values ");
            ...
            sb.AppendLine("\tBuffer Options: bip(def, buffer if possible) buf(buffer) dnb(do not buffer)");
            ...
            sb.AppendLine("-delete <tagmasks> <starttime,endtime>[,Buffer options] #delete archive data");
